...
export default function ResultatsClient({ resultats, saisons, locale }: Props) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {/* Filters */}
      <div className="flex flex-wrap gap-6 mb-12 border-b border-white/[0.06] pb-8">
        <div>
          ...
          <select
            ...
            className="bg-bg-surface border border-white/[0.08] text-white text-sm px-3 py-2 outline-none focus:border-royal appearance-none pr-8"
          >
      ...
      <div className="mb-12">
        {(['or', 'argent', 'bronze'] as const).map(key => (
          <div key={key} className="border-t border-white/[0.06] group">
            <div className="py-5 flex items-center gap-8">
              ...
              <div className="flex items-center gap-3 border-l border-white/[0.08] pl-8">
                ...
                <span className="font-heading text-lg md:text-xl text-white tracking-wider uppercase">
                  {medalLabel[key]}
                </span>
              </div>
            </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div>
          {filtered.map(r => (
            <div
              key={r.id}
              className="border-t border-white/[0.06] py-4 flex items-center gap-4 group hover:border-royal/30 transition-colors duration-200"
            >
              <span className={cn('w-1.5 h-1.5 shrink-0', medalDot[r.medaille])} />
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-medium truncate">{r.athlete}</p>
